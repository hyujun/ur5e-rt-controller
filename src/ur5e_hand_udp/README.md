...
- **퍼블리시**: `/hand/joint_states` (`std_msgs/Float64MultiArray`, 100Hz)
  - 77개 `double` 값 (motor_positions[11] + motor_velocities[11] + motor_currents[11] + sensor_data[44])
...
| 토픽 | 타입 | 방향 | 설명 |
|------|------|------|------|
| `/hand/joint_states` | `std_msgs/Float64MultiArray` | 퍼블리시 | 77개 손 상태값 (100Hz) |
| `/hand/command` | `std_msgs/Float64MultiArray` | 구독 | 11개 정규화된 손 명령 |
...
ur5e_rt_controller::HandUdpReceiver receiver(50001);
receiver.Start([](const ur5e_rt_controller::HandState& state) {
  // 콜백: UDP 패킷 수신 시 호출
  if (state.valid) {
    // state.motor_positions[11]
    // state.motor_velocities[11]
    // state.motor_currents[11]
    // state.sensor_data[44]
  }
});
