...
```cpp
struct ThreadConfig {
  int cpu_core{-1};              // CPU 코어 번호 (-1 = 미설정)
  int sched_policy{SCHED_OTHER}; // SCHED_FIFO / SCHED_RR / SCHED_OTHER
  int sched_priority{0};         // 우선순위 (FIFO: 1–99)
  int nice_level{0};             // nice 레벨 (SCHED_OTHER 전용)
  std::string_view thread_name{};// 스레드 이름 (최대 15자)
};
```

#### 사전 정의 ThreadConfig 상수 (6코어 시스템)

| 상수 | 코어 | 정책 | 우선순위 | 용도 |
|------|------|------|----------|------|
| `kRtControlConfig` | 2 | `SCHED_FIFO` | 90 | 500Hz 제어 루프 |
| `kSensorConfig` | 3 | `SCHED_FIFO` | 70 | 센서 구독 스레드 |
| `kUdpRecvConfig` | 3 | `SCHED_FIFO` | 65 | UDP 수신 스레드 |
| `kLoggingConfig` | 4 | `SCHED_OTHER` | nice -5 | CSV 로깅 스레드 |
| `kAuxConfig` | 5 | `SCHED_OTHER` | 0 | 보조 스레드 |

**4코어 대체 상수**: `kRtControlConfig4Core` (Core 1), `kSensorConfig4Core` (Core 2), `kLoggingConfig4Core` (Core 3)

---

### `ur5e_rt_base/thread_utils.hpp`

RT 스레드 설정을 적용하고 검증하는 유틸리티 함수를 제공합니다.

```cpp
// CPU 친화성, 스케줄러 정책, 우선순위, 스레드 이름을 한 번에 적용
// 권한 부족 시 false 반환 (노드는 SCHED_OTHER로 계속 동작)
[[nodiscard]] bool ApplyThreadConfig(const ThreadConfig& config);

// 현재 스레드 설정이 config와 일치하는지 검증
[[nodiscard]] bool VerifyThreadConfig(const ThreadConfig& config);
```
